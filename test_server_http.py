...
class HttpStartupConfigTests(unittest.TestCase):
    def test_main_starts_when_mcp_api_token_is_configured(self):
        ...
        self.assertEqual(("127.0.0.1", server_http.PORT), server.server_address)
    ...
    def test_http_bind_host_defaults_to_loopback_and_allows_explicit_wildcard(self):
        self.assertEqual("127.0.0.1", server_http.validate_http_bind_host(None))
        self.assertEqual("0.0.0.0", server_http.validate_http_bind_host(" 0.0.0.0 "))
        with self.assertRaisesRegex(RuntimeError, "MCP_BIND_HOST"):
            server_http.validate_http_bind_host("192.168.1.15")
...
class OAuthFlowTests(unittest.TestCase):
    ...
    def test_authenticated_get_mcp_returns_405_when_sse_is_not_supported(self):
        server, thread, base = self._start_server()
        try:
            ...
            self.assertEqual("POST, OPTIONS", raised.exception.headers["Allow"])
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=5)
    ...
    def test_streamable_http_initialize_then_tools_list(self):
        server, thread, base = self._start_server()
        try:
            common_headers = {
            ...
            initialize_body = json.dumps({
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "regression-client", "version": "1.0"},
                },
            }).encode("utf-8")
            initialize_request = urllib.request.Request(
                f"{base}/mcp", data=initialize_body, headers=common_headers, method="POST"
            )
            ...
            notification_request = urllib.request.Request(
                f"{base}/mcp", data=notification, headers=notification_headers, method="POST"
            )
            ...
            tools_request = urllib.request.Request(
                f"{base}/mcp", data=tools_body, headers=notification_headers, method="POST"
            )
    ...
    def test_initialize_negotiates_supported_version_instead_of_echoing_unknown(self):
        response = server_http.handle_initialize(
            1,
            {
        ...
        self.assertEqual(server_http.PROTOCOL_VERSION, response["result"]["protocolVersion"])

    def test_streamable_http_rejects_incomplete_accept_header(self):
        server, thread, base = self._start_server()
        try:
            body = json.dumps({
            ...
            self.assertEqual(406, raised.exception.code)
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=5)
    ...
    @staticmethod
    def _modern_request_body(method: str, request_id: int = 1, **params) -> bytes:
        modern_meta = {
            "io.modelcontextprotocol/protocolVersion": server_http.LATEST_PROTOCOL_VERSION,
            "io.modelcontextprotocol/clientInfo": {"name": "modern-regression", "version": "1.0"},
    ...
    @staticmethod
    def _modern_headers(method: str, **extra_headers) -> dict:
        return {
            "Authorization": "Bearer secret-token",
            "Content-Type": "application/json",
            "Accept": "application/json, text/event-stream",
    ...
    def test_modern_streamable_http_discover_then_tools_list(self):
        server, thread, base = self._start_server()
        try:
            discover_request = urllib.request.Request(
                f"{base}/mcp",
                data=self._modern_request_body("server/discover"),
                headers=self._modern_headers("server/discover"),
                method="POST",
            )
            with urllib.request.urlopen(discover_request, timeout=5) as response:
            ...
            self.assertEqual([server_http.LATEST_PROTOCOL_VERSION], discover_result["supportedVersions"])
            self.assertEqual(server_http.SERVER_INFO, discover_result["_meta"]["io.modelcontextprotocol/serverInfo"])
            self.assertNotIn("serverInfo", discover_result)

            tools_request = urllib.request.Request(
                f"{base}/mcp",
                data=self._modern_request_body("tools/list", request_id=2),
                headers=self._modern_headers("tools/list"),
                method="POST",
            )
            with urllib.request.urlopen(tools_request, timeout=5) as response:
                listed = json.loads(response.read().decode("utf-8"))
            tools_result = listed["result"]
            self.assertEqual("complete", tools_result["resultType"])
            self.assertEqual(server_http.SERVER_INFO, tools_result["_meta"]["io.modelcontextprotocol/serverInfo"])
    ...
    def test_modern_streamable_http_rejects_missing_or_mismatched_standard_headers(self):
        server, thread, base = self._start_server()
        try:
            cases = [
                (
                    "missing-version",
                    self._modern_request_body("server/discover"),
                    {
                        "Authorization": "Bearer secret-token",
                        "Content-Type": "application/json",
                        "Accept": "application/json, text/event-stream",
                ...
                (
                    "method-mismatch",
                    self._modern_request_body("tools/list"),
                    self._modern_headers("ping"),
                ),
                (
                    "missing-name",
                    self._modern_request_body("tools/call", name="fs_disk_info", arguments={}),
                    self._modern_headers("tools/call"),
                ),
            ]
    ...
    def test_modern_streamable_http_rejects_unknown_protocol_version_with_supported_list(self):
        server, thread, base = self._start_server()
        try:
            body = self._modern_request_body("tools/list")
            headers = self._modern_headers("tools/list")
            ...
            self.assertEqual("2099-01-01", payload["error"]["data"]["requested"])
            self.assertIn(server_http.LATEST_PROTOCOL_VERSION, payload["error"]["data"]["supported"])
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=5)

    def test_modern_mcp_name_base64_sentinel_decodes_before_comparison(self):
        encoded = "=?base64?ZnNfZGlza19pbmZv?="
        self.assertEqual("fs_disk_info", MCPHTTPHandler._decode_mcp_header_value(encoded))
        with self.assertRaises(ValueError):
            MCPHTTPHandler._decode_mcp_header_value("=?base64?not-valid?=")
...
class CloudflareAccessModeTests(unittest.TestCase):
    ...
    def test_public_mcp_rejects_static_bearer_without_explicit_fallback(self):
        server, thread, base = self._start_server()
        try:
            body = json.dumps({
                "jsonrpc": "2.0",
            ...
            with self.assertRaises(urllib.error.HTTPError) as raised:
                urllib.request.urlopen(req, timeout=5)
            self.assertEqual(401, raised.exception.code)
            self.assertNotIn("resource_metadata", raised.exception.headers["WWW-Authenticate"])
        finally:
            server.shutdown()
            server.server_close()
            thread.join(timeout=5)

    def test_public_mcp_allows_static_bearer_only_when_explicit_fallback_is_enabled(self):
        config = HandcraftServerConfig(
            mcp_api_token="secret-token",
            base_url="https://mcp.example.test",
            cloudflare_access_enabled=True,
    ...
    def test_public_mcp_get_with_bearer_returns_405_when_sse_is_not_supported(self):
        config = HandcraftServerConfig(
            mcp_api_token="secret-token",
            base_url="https://mcp.example.test",
            cloudflare_access_enabled=True,
...
class HonchoMcpFacadeTests(unittest.TestCase):
    ...
    def test_edgars_mcp_tools_list_includes_prefixed_honcho_tools(self):
        captured = {}
        ...
        class FakeUpstreamResponse:
            ...
            def read(self):
                return (
                    b'event: message\n'
                    b'data: {"jsonrpc":"2.0","id":"edgars-mcp-honcho-tools-list",'
                    b'"result":{"tools":[{"name":"inspect_workspace","description":"Inspect memory",'
        ...
        try:
            ...
            honcho_tool = next(tool for tool in tools if tool["name"] == "honcho__inspect_workspace")
            self.assertEqual(200, response.status)
            self.assertEqual("Honcho: Inspect Workspace", honcho_tool["title"])
            self.assertIn("[Honcho memory]", honcho_tool["description"])
            self.assertEqual(
                {"upstream": "honcho", "upstream_name": "inspect_workspace"},
                honcho_tool["_meta"]["edgars_mcp_proxy"],
            )
            self.assertEqual("tools/list", captured["data"]["method"])
    ...
    def test_edgars_mcp_honcho_prefixed_tool_call_uses_upstream_tool_name(self):
        captured = {}
        ...
        try:
            body = json.dumps({
                ...
                "method": "tools/call",
                "params": {
                    "name": "honcho__inspect_workspace",
                    "arguments": {"include": "peers"},
                },
            }).encode("utf-8")
            req = urllib.request.Request(
                f"{base}/mcp",
            ...
            self.assertEqual(200, response.status)
            self.assertEqual("workspace ok", payload["result"]["content"][0]["text"])
            self.assertEqual("tools/call", captured["data"]["method"])
            self.assertEqual("inspect_workspace", captured["data"]["params"]["name"])
            self.assertEqual({"include": "peers"}, captured["data"]["params"]["arguments"])
            self.assertEqual("Bearer honcho-secret", captured["headers"]["Authorization"])
        finally:
            server.shutdown()
            server.server_close()
    ...
    def test_normalize_honcho_mcp_response_preserves_multi_event_sse(self):
        body = (
            b'event: message\n'
            b'data: {"jsonrpc":"2.0","id":1,"result":{"cursor":"page-1"}}\n\n'
            b'event: message\n'
            b'data: {"jsonrpc":"2.0","id":1,"result":{"cursor":"page-2"}}\n\n'
        )

        normalized, content_type = server_http.normalize_honcho_mcp_response(body, "text/event-stream")
    ...
    def test_honcho_mcp_hostname_get_uses_standard_mcp_path(self):
        captured = {}
        ...
        class FakeUpstreamResponse:
            ...
            def read(self):
                return b'{"ok":true,"server":"honcho"}'
        ...
        try:
            ...
            self.assertEqual(200, response.status)
            self.assertEqual({"ok": True, "server": "honcho"}, payload)
            self.assertEqual(server_http.HONCHO_MCP_UPSTREAM_URL, captured["url"])
