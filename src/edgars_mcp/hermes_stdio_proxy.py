"""
Hermes stdio bridge for the edgars-mcp HTTP endpoint.

Hermes can launch local MCP servers over stdio. This proxy keeps that local
stdio contract while forwarding JSON-RPC requests to edgars_mcp.http_server.
...
MCP_URL = os.getenv("EDGARS_MCP_MCP_URL", DEFAULT_MCP_URL)
REQUEST_TIMEOUT_SECONDS = float(os.getenv("EDGARS_MCP_TIMEOUT_SECONDS", "30"))
PREFLIGHT_TIMEOUT_SECONDS = float(os.getenv("EDGARS_MCP_PREFLIGHT_TIMEOUT_SECONDS", "5"))
...
def load_auth_token() -> str:
    for env_name in ("EDGARS_MCP_MCP_TOKEN", "MCP_API_TOKEN", "MCP_AUTH_TOKEN"):
...
def run_preflight() -> None:
    ...
    try:
        ...
        if exc.code == 401:
            raise HermesPreflightError(describe_preflight_unauthorized_error()) from exc
        raise HermesPreflightError(f"edgars-mcp preflight failed at {MCP_URL}: HTTP {exc.code}") from exc
    except urllib.error.URLError as exc:
        raise HermesPreflightError(f"edgars-mcp is unreachable at {MCP_URL}: {exc.reason}") from exc
    except TimeoutError as exc:
        raise HermesPreflightError(f"edgars-mcp preflight timed out at {MCP_URL}") from exc
    except json.JSONDecodeError as exc:
        raise HermesPreflightError(f"edgars-mcp preflight returned non-JSON from {MCP_URL}") from exc

    if not isinstance(response, dict) or "result" not in response:
        raise HermesPreflightError(f"edgars-mcp preflight returned an invalid initialize response from {MCP_URL}")
