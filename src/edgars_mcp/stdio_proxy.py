"""
Stdio bridge for the edgars-mcp HTTP endpoint.

This proxy lets any MCP client that launches local servers over stdio (Claude
Desktop / Cursor / Hermes / etc.) talk to edgars-mcp, which is served by
edgars_mcp.http_server over HTTP. It forwards JSON-RPC lines from stdin to the HTTP
...
# Prefer generic env names; keep EDGARS_MCP_* as backward-compat fallback.
MCP_URL = os.getenv("MCP_URL", os.getenv("EDGARS_MCP_MCP_URL", DEFAULT_MCP_URL))
REQUEST_TIMEOUT_SECONDS = float(
    os.getenv("MCP_REQUEST_TIMEOUT_SECONDS",
              os.getenv("EDGARS_MCP_TIMEOUT_SECONDS", "30"))
)
PREFLIGHT_TIMEOUT_SECONDS = float(
    os.getenv("MCP_PREFLIGHT_TIMEOUT_SECONDS",
              os.getenv("EDGARS_MCP_PREFLIGHT_TIMEOUT_SECONDS", "5"))
)
...
def load_auth_token() -> str:
    # Prefer generic names; keep EDGARS_MCP_MCP_TOKEN for backward compat.
    for env_name in ("MCP_API_TOKEN", "MCP_AUTH_TOKEN", "EDGARS_MCP_MCP_TOKEN"):
...
def load_cf_access_service_token() -> tuple[str, str]:
    client_id_names = (
        ...
        "CF_ACCESS_CLIENT_ID",
        "EDGARS_MCP_CF_ACCESS_CLIENT_ID",
    )
    client_secret_names = (
        "MCP_CF_ACCESS_CLIENT_SECRET",
        "CF_ACCESS_CLIENT_SECRET",
        "EDGARS_MCP_CF_ACCESS_CLIENT_SECRET",
    )
...
def build_request(payload: dict) -> urllib.request.Request:
    ...
    headers = {
        ...
        "X-Edgars-Mcp-Client-Mode": "stdio-local",
    }
...
def validate_auth_token() -> str:
    ...
    raise PreflightError(
        "No MCP auth is available for edgars-mcp preflight. Set MCP_API_TOKEN for localhost or MCP_CF_ACCESS_CLIENT_ID and MCP_CF_ACCESS_CLIENT_SECRET for Access-protected public MCP."
    )
...
def describe_preflight_unauthorized_error() -> str:
    ...
    if has_bearer and has_service_token:
        return (
            f"edgars-mcp rejected preflight auth at {MCP_URL}. "
            "Check MCP_API_TOKEN, MCP_CF_ACCESS_CLIENT_ID / MCP_CF_ACCESS_CLIENT_SECRET, and Cloudflare Access configuration."
        )
    if has_bearer:
        return f"MCP_API_TOKEN was rejected by edgars-mcp at {MCP_URL}"
    if has_service_token:
        return (
            f"Cloudflare Access service token was rejected by edgars-mcp at {MCP_URL}. "
            "Check MCP_CF_ACCESS_CLIENT_ID / MCP_CF_ACCESS_CLIENT_SECRET and Cloudflare Access configuration."
        )
    return f"edgars-mcp rejected preflight auth at {MCP_URL}"
...
def run_preflight() -> None:
    ...
    try:
        ...
        raise PreflightError(f"edgars-mcp preflight failed at {MCP_URL}: HTTP {exc.code}") from exc
    except urllib.error.URLError as exc:
        raise PreflightError(f"edgars-mcp is unreachable at {MCP_URL}: {exc.reason}") from exc
    except TimeoutError as exc:
        raise PreflightError(f"edgars-mcp preflight timed out at {MCP_URL}") from exc
    except json.JSONDecodeError as exc:
        raise PreflightError(f"edgars-mcp preflight returned non-JSON from {MCP_URL}") from exc

    if not isinstance(response, dict) or "result" not in response:
        raise PreflightError(f"edgars-mcp preflight returned an invalid initialize response from {MCP_URL}")
