...
from edgars_mcp import http_server as server_http
from edgars_mcp.config import RuntimePaths
from edgars_mcp.http_server import (
    JOBS,
    JOBS_LOCK,
    DISCORD_WEBHOOK_EVENTS,
    DISCORD_WEBHOOK_EVENTS_LOCK,
    EdgarsMcpServerConfig,
    MCPHTTPHandler,
    ThreadingHTTPServer,
    TOOLS,
...
_TEST_RUNTIME_DIR = tempfile.mkdtemp(prefix="edgars-mcp-tests-")
unittest.addModuleCleanup(shutil.rmtree, _TEST_RUNTIME_DIR, True)
server_http.RUNTIME_PATHS = RuntimePaths.from_env(
    {"EDGARS_MCP_RUNTIME_DIR": _TEST_RUNTIME_DIR}
)
server_http.LINEAR_OAUTH_TOKEN_FILE = server_http.RUNTIME_PATHS.state / "linear-oauth-token.json"
...
class HttpStartupConfigTests(unittest.TestCase):
    def test_load_secret_reads_mounted_secret_file(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            secret_file = Path(tmpdir) / "warp_api_key"
            secret_file.write_text("warp-test-key\n", encoding="utf-8")
            with patch.dict(
                "os.environ",
                {"WARP_API_KEY_FILE": str(secret_file)},
                clear=True,
            ):
                self.assertEqual("warp-test-key", server_http.load_secret("WARP_API_KEY"))
    ...
    def test_main_starts_when_mcp_api_token_is_configured(self):
        ...
        with patch.dict(
            ...
            clear=True,
        ), patch("edgars_mcp.http_server.ThreadingHTTPServer", FakeServer):
            server_http.main()
        ...
        for label, environment in cases.items():
            with self.subTest(label=label):
                with patch.dict("os.environ", environment, clear=True), patch(
                    "edgars_mcp.http_server.ThreadingHTTPServer"
                ) as server_class:
    ...
    def test_http_servers_keep_separate_auth_config(self):
        first_config = EdgarsMcpServerConfig(mcp_api_token="first-token", base_url="https://first.example")
        second_config = EdgarsMcpServerConfig(mcp_api_token="second-token", base_url="https://second.example")
...
class OAuthFlowTests(unittest.TestCase):
    ...
    def _start_server(self, config=None):
        config = config or EdgarsMcpServerConfig(
            mcp_api_token="secret-token",
            base_url="https://mcp.example.test",
        )
        server = ThreadingHTTPServer(("127.0.0.1", 0), MCPHTTPHandler, config=config)
    ...
    def test_static_edgars_mcp_client_pkce_allows_missing_client_secret(self):
        server, thread, base = self._start_server()
        try:
            redirect_uri = "https://chatgpt.com/connector/oauth/callback"
    ...
    def test_package_webhook_can_require_shared_secret(self):
        ...
        try:
            ...
            authed_req = urllib.request.Request(
                ...
                headers={
                    "Content-Type": "application/json",
                    "X-Edgars-Mcp-Webhook-Token": "pkg-secret",
                },
                method="POST",
            )
...
class ClaudeCodeAgentSmokeTests(unittest.TestCase):
    ...
    def test_claude_code_agent_normal_tool_call_example(self):
        ...
        command = args[0]
        self.assertEqual(
            server_http.cli_command(
                server_http.CLAUDE_CMD, "-p", "say hi", "--output-format", "text"
            ),
            command,
        )
