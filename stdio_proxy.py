...
def build_request(payload: dict) -> urllib.request.Request:
    ...
    if NEGOTIATED_PROTOCOL_VERSION and payload.get("method") != "initialize":
...
def forward_to_http(payload: dict, timeout: float = REQUEST_TIMEOUT_SECONDS) -> dict | None:
    global NEGOTIATED_PROTOCOL_VERSION
    ...
    with urllib.request.urlopen(request, timeout=timeout) as response:
        ...
        if payload.get("method") == "initialize":
            NEGOTIATED_PROTOCOL_VERSION = str((parsed.get("result") or {}).get("protocolVersion") or "")
        return parsed
